...
def run_hybrid_retrieval(
    ...
    retriever = HybridRetrieval(tokenizer, p_encoder_path, q_encoder_path) 
    ...
    if data_args.use_faiss:
        ...
        doc_score, doc_indices = retriever.get_relevant_doc_bulk(
            datasets["validation"].to_pandas()["question"].to_list(), data_args.top_k_retrieval
        )
        df = run_dense_retrieval(
            tokenizer,
            datasets,
            doc_indices=doc_indices,
            id2context_dict=hybrid_retrieval.id2context_dict,
...
def run_dense_retrieval(
    tokenizer,
    datasets: DatasetDict,
    doc_indices, id2context_dict, topk, ########################### 여기 손보기!
    training_args: TrainingArguments,
    data_args: DataTrainingArguments,
    data_path: str = "../data",
    context_path: str = "wikipedia_documents.json",
    ...
    for i in range(len(datasets["validation"]["id"])):
        texts = []
        for j in range(len(doc_indices[question_texts[i]])):
            texts.append(
                id2context_dict[doc_indices[question_texts[i]][j]])
